...
#region UnityMethods:
	...
	private void OnDrawGizmos()
	{
		...
		{
			UpdateViewportPlane(voidlessCamera.camera.nearClipPlane, ref nearPlane);
	...
	public void UpdateViewportPlane(float z, ref CameraViewportPlane _plane)
	{
		...
		float y = (Mathf.Tan(voidlessCamera.camera.fieldOfView * Mathf.Deg2Rad * 0.5f) * z);
		float x = y * voidlessCamera.camera.aspect;
	...
	{
		if(_calculateViewportPoint) _point = voidlessCamera.camera.WorldToViewportPoint(_point);
	...
	private void DrawCameraGrid()
	{
		...
		Axes2D axesInside = Axes2DWithinGridFocusArea(voidlessCamera.GetTargetPosition());
