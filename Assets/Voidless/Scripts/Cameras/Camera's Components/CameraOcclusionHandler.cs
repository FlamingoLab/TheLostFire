...
#region UnityMethods:
	...
	private void OnDrawGizmos()
	{
		...
		if(drawGizmos)
		{
			...
			CameraViewportPlane nearPlane = voidlessCamera.viewportHandler.NearPlane;

			Gizmos.DrawWireSphere(voidlessCamera.position, radius);

			foreach(Vector3 point in voidlessCamera.viewportHandler)
			{
				Gizmos.DrawSphere(Vector3.LerpUnclamped(voidlessCamera.viewportHandler.gridAttributes.center, point, pointRatio), gizmosSphereRadius);
			}
		}	
#endif
	}
	...
	public bool HitBetweenViewportPoints(Vector3 _point)
	{
		...
		foreach(Vector3 point in voidlessCamera.viewportHandler)
		{
			rayOrigin = Vector3.LerpUnclamped(voidlessCamera.viewportHandler.gridAttributes.center, point, pointRatio);
	...
	public float CalculateAdjustedDistance(Vector3 _point)
	{
		...
		foreach(Vector3 point in voidlessCamera.viewportHandler)
		{
			direction = (Vector3.LerpUnclamped(voidlessCamera.viewportHandler.gridAttributes.center, point, pointRatio) - _point);
		...
		if(Physics.SphereCast(voidlessCamera.position, radius, transform.forward, out hit, radius, hitMask)) distance -= radius;
