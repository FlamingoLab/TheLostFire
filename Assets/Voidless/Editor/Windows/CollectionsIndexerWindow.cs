...
public class CollectionsIndexerWindow : EditorWindow
{
	...
	private void DrawObjectsField()
	{
		...
		for(int i = 0; i < CollectionsIndexer.objects.Count; i++)
		{
			...
			int itemIndex = 0;
			IEnumerator<Tuple<string, IEnumerable<Object>>> iterator = CollectionsIndexer.GetEnumerator(i);
			...
			{
				Tuple<string, IEnumerable<Object>> tuple = iterator.Current;
				itemIndex = 0;
				
				GUILayout.Label("Collection " + collectionIndex + " - " + tuple.value1.ToInspectorFormat() + ": ");

				EditorGUILayout.Space();

				foreach(Object obj in tuple.value2)
				{
