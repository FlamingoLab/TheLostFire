...
public class CollectionsIndexer
{
	...
	public static Dictionary<int, List<Tuple<string, IEnumerable<Object>>>> objectsCollections; 	/// <summary>Objects' Collections.</summary>
	public static bool loaded;

	/// <summary>Initializes Collections' Indexer.</summary>
	public static void Initialize()
	{
		objectsCollections = new Dictionary<int,List<Tuple<string, IEnumerable<Object>>>>();
	}
	...
	public static void Update()
	{
		...
		List<Tuple<string, IEnumerable<Object>>> tupleCollections = null;
		int ID = 0;
		...
		foreach(Object obj in objects)
		{
			...
			tupleCollections = new List<Tuple<string, IEnumerable<Object>>>();
			ID = obj.GetInstanceID();

			foreach(FieldInfo info in fieldsInfo)
			{
				object fieldValue = info.GetValue(obj);
				Tuple<string, IEnumerable<Object>> tuple = new Tuple<string, IEnumerable<Object>>(null, null);

				if(fieldValue == null) continue;

				if(fieldValue is IEnumerable<Object>)
				{
					tuple.value1 = info.Name;
					tuple.value2 = fieldValue as IEnumerable<Object>;
				}

				if(tuple.value2 != null)
				{
					tupleCollections.Add(tuple);
					foreach(Object asset in tuple.value2)
					{
	...
	public static string[] GetObjectCollectionsNames(int _index)
	{
		...
		{
			foreach(Tuple<string, IEnumerable<Object>> tuple in objectsCollections[ID])
			{
				collectionNames.Add(tuple.value1.ToInspectorFormat());
			}
		}
	...
	public static string[] GetObjectCollectionsItems(int _objectIndex, int _collectionIndex)
	{
		...
		if(objectsCollections.ContainsKey(ID))
		{
			foreach(Object obj in objectsCollections[ID][_collectionIndex].value2)
			{
	...
	public static IEnumerator<Tuple<string, IEnumerable<Object>>> GetEnumerator(int _index)
	{
		int ID = objects[_index].GetInstanceID();
